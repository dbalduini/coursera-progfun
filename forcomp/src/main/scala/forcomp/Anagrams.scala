...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    val cmap = (w.toLowerCase groupBy ((c: Char) => c)) map {
      case (x, y) => x -> y.length
    }
    cmap.toList.sorted
  }
  ...
  def wordAnagrams(word: Word): List[Word] = {
    val d = dictionaryByOccurrences withDefaultValue List()
    d(wordOccurrences(word))
  }
  ...
  //          i <- 1 to comb._2
  //        } yield {
  //          val combs = combine(xs drop n)
  //          List((comb._1, i)) ::: combs
  //        }
  //      }.toList
  //    combine(occurrences)
  //  }
    ...
    def loop(occurrences: Occurrences): List[Sentence] =
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          rest <- loop(subtract(occurrences, wordOccurrences(word)))
        } yield word :: rest
