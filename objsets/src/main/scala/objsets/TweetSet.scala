...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def iter(set: TweetSet, acc: TweetList): TweetList = {
      if (set.isEmpty) acc
      else {
        val head = set.mostRetweeted
        def tail = set.remove(head)
        iter(tail, acc incl new Cons(head, Nil))
      }
    }
    iter(this, Nil)
  }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def toString = "{" + left + elem.retweets + right + "}"

  def isEmpty = false

  def mostRetweeted: Tweet = {
    val other = filter(_.retweets > elem.retweets)
    if (other.isEmpty) elem
    else if (other.mostRetweeted.retweets >= elem.retweets) other.mostRetweeted else elem
  }
...
object Nil extends TweetList {
  ...
  def isEmpty = true
  def incl(that: TweetList): TweetList = that
}

class Cons(val head: Tweet, val tail: TweetList) extends TweetList {
  def isEmpty = false
  def incl(that: TweetList): TweetList = new Cons(head, tail incl that)
}
